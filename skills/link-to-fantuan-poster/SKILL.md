...
assets/layout.json 集中管理区域、字体、黑色标题、蓝紫渐变和二维码位置。macOS 自动定位苹方；其他系统设置 font 为可用中文字体路径。二维码为 M 级纠错、四模块白边、整数倍缩放。通过标题的位置、字号与行距调整留白；用户只要求改善留白时，不添加插画、卡片或装饰元素。背景保留饭团、电脑、绿植、书本及虚线框；无标题蓝底、副标题框或摘要卡片。底图修改使用图像编辑工具，保留原始生成记录并重新核对排版。
