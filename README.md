...
`skills/link-to-fantuan-poster/assets/layout.json` 集中配置字号、颜色、区域和字体。底图固定，日常生成不调用图像生成服务。通过标题位置、字号和行距改善留白，不额外添加插画元素。
...
`skills/link-to-fantuan-poster/assets/layout.json` centralizes font sizes, colors, regions and font selection. Routine rendering reuses the background without calling an image generation service. Headline position, size and line spacing balance the whitespace without adding illustration elements.
